...
public interface EndpointHitRepository extends JpaRepository<EndpointHit, Integer> {
     ...
     */
    @Query("select new ru.practicum.ewm.stats.server.model.ViewStats(eh.app, eh.uri, COUNT(eh.id)) " +
            "from EndpointHit eh " +
     ...
     */
    @Query("select new ru.practicum.ewm.stats.server.model.ViewStats(eh.app, eh.uri, COUNT(DISTINCT eh.ip)) " +
            "from EndpointHit eh " +
            "where eh.timestamp between :start and :end " +
            "and (:uris is null or eh.uri in :uris) " +
            "group by eh.app, eh.uri " +
